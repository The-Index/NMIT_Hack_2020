...
import requests
import json
#Python script 8765to start a websocket server on LAN wifi/port number .
lan_ip = sys.argv[1]
message = ''
#lock = threading.Lock()

data = requests.get('http://api.ipstack.com/'+ip+'?access_key=7d0e0642b88c21497c80bbb570f74bbc')
...
def fire_detect(lock):
    ...
    stream=urllib.request.urlopen('http://192.168.225.135:8000/video_feed')
    total_bytes = b''
    while(True):
        global message
        global lat
        ...
        if not b == -1:
            ...
            detections = detector.detectObjectsFromImage(input_image=os.path.join(execution_path, "img.jpg"),
                                                 output_type='array',
...
#     detector.setJsonPath(configuration_json=os.path.join(execution_path, "detection_config.json"))
#     detector.loadModel()
#     cap = cv2.VideoCapture('fires2.mp4')
            
#     def forFrame(frame_number, output_array, output_count,img_array):
#         global message
#         lock.acquire()
#         #print(type(img_array))        
#         cv2.imshow('fires',img_array)
#         if output_array:
#             message = 'fire,37.7510,-97.8220'
#         else:
#             message = 'no fire,37.7510,-97.8220'
#         lock.release()
#         k=cv2.waitKey(1)
#         if k == 27:
#             exit()
                
#     detector.detectObjectsFromVideo(camera_input=cap, 
